...
public:
    ...
    short Search(fileEntry data);
    short Search(const char* name);
...
public:
    ...
    Directory get(short inode_id);
    void put(short inode_id, Directory directory);

private:
    typedef std::list<short>::iterator ListIt;
    std::unordered_map<short, std::pair<Directory, ListIt>> cache;
    std::list<short> used;
    size_t capacity;

    void touch(std::unordered_map<short, std::pair<Directory, ListIt>>::iterator it);
};
